 ...
 */
static void curve25519_pack(uint8_t o[CURVE25519_KEY_SIZE], const gf n) {
	gf t;
 ...
 */
static void curve25519_unpack(gf o, const uint8_t n[CURVE25519_KEY_SIZE]) {
	for (uint_fast8_t i = 0; i < 16; i++) {
...
void curve25519_scalarmult(
	uint8_t result[CURVE25519_KEY_SIZE],
	const uint8_t priv_key[CURVE25519_KEY_SIZE],
...
int main()
{
	...
	for (int i = 0; i < 16; ++i) {
		uint8_t t = testkey[i];
		testkey[i] = testkey[31 - i];
		testkey[31 - i] = t;
	}

	curve25519_scalarmult_base(result, testkey);
